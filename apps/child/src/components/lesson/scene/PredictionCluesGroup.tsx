...
export function PredictionCluesGroup({ clue, prediction }: PredictionCluesGroupProps) {
  return (
    <group>
      <Html position={[0, 1, 0]} center style={{ width: "440px" }}>
        <p className="text-center text-lg font-bold text-white">{clue}</p>
      </Html>
      ...
      <Html position={[0, -1, 0]} center style={{ width: "440px" }}>
        <p className="text-center text-lg font-extrabold text-accent">{prediction}</p>
      </Html>
    </group>
  );
}
